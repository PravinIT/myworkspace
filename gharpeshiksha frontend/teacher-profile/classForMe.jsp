...
TutorDetail tutor=ofy().load().type(TutorDetail.class).id(((Long)session.getAttribute("tutor")).longValue()).now();				 						
	int dist_check=tutor.getRadius()>10000?tutor.getRadius()/1000:10;	
	 
double fromlat,tolat,fromlng,tolng;
fromlat=tutor.getLatitude()-dist_check*0.008993182000001099;
tolat=tutor.getLatitude()+dist_check*0.008993182000001099;
fromlng=tutor.getLongitude()-dist_check*0.010258364990164637;
tolng=tutor.getLongitude()+dist_check*0.010258364990164637; 
long total_enq=ofy().load().type(Lead.class).count()+2283+4;
List<Lead> leads=ofy().load().type(Lead.class).order("-enq_id").limit(500).list();
...
List<Subjects> list=ofy().load().type(Subjects.class).filter("tutor", Ref.create(tutor)).list();
Iterator<Subjects> itrsub=list.iterator();
while(itrsub.hasNext())
	subs.add(itrsub.next().getCourse());
%>
...
<%
    Lead ld=null;
    List<LeadSubjects> ldsublist=null;
    Iterator<LeadSubjects> ldsubitr=null;
    LeadSubjects ldsub=null;
    Subjects sub=null;
    List<String> tutorsub=null;
    double distance;
   ...
   while(itr.hasNext()){
    	 	expression=false;
   ld=itr.next();
if(ld.getLongitude()>=fromlat&&ld.getLongitude()<=tolat&&ld.getLatitude()>=fromlng&&ld.getLatitude()<=tolng&&(ld.getSex_pref().equals("any")||ld.getSex_pref().equals(tutor.getGender())))
{   distance=Distance.distance(ld.getLongitude(), ld.getLatitude(), tutor.getLatitude(), tutor.getLongitude());
if(distance<dist_check*1000){

  	ldsublist=ofy().load().type(LeadSubjects.class).filter("enq_id", Ref.create(ld)).list();
       ldsubitr=ldsublist.iterator();
    while(ldsubitr.hasNext())
    { ldsub=ldsubitr.next();
    if(ldsub.getSubjects()==null)
    	continue;
    if(subs.contains(ldsub.getCourse()))
    { 
    sub=ofy().load().type(Subjects.class).filter("tutor", Ref.create(tutor)).filter("course", ldsub.getCourse()).first().now();
	if(sub.getSubjects()==null)
		continue;
	tutorsub= Arrays.asList(sub.getSubjects());
	 for(String s:ldsub.getSubjects())
	{  
		if(tutorsub.contains(s))
		{
				...
				%>
                 ...
                 border-left: 1px solid #cecece;  border-bottom: 1px solid #cecece;
                  padding: 10px;">
                                <div class="row">
                                    <div class="col-md-6">
                                      ...
                                      <%if(!ld.getStatus().equals("Cancelled")){ 
                                        ...
                                        <a href="#" data-toggle="modal" data-target="#emailModal" target="_blank" id="<%=ld.getEnq_id()%>" onClick="getidforemail(this.id)" data-toggle="tooltip" title="Share via email">
                                            <i class="fas fa-envelope fa-2x text-dark" style="margin-left: 7px;color:grey"></i>
                                        </a>
                                        <a href="#" data-toggle="modal" data-target="#messageModal" id="<%=ld.getEnq_id()%>" onClick="getidformsg(this.id)" target="_blank" data-toggle="tooltip" title="Share via text message">
                         ...
                         <% }if(expression)break;} } if(expression)break; } } } }
                                                        ...
                                                        <div class="row">
                                                            ...
                                                            <div class="col-md-4 text-center">
                                                                <div class="panel panel-danger panel-pricing" style="border-color:#b8d7ef;">
                                                                    ...
                                                                    <div class="panel-footer">
                                                                        <a class="btn btn-lg btn-block btn-primary" href="payment.jsp?amt=3000&tut_id=<%=tutor.getTut_id()%>">BUY NOW</a>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <!-- /item -->
                                                            ...
                                                            <div class="col-md-4 text-center">
                                                                <div class="panel panel-warning panel-pricing">
                                                                    ...
                                                                    <div class="panel-footer">
                                                                        <a class="btn btn-lg btn-block btn-warning" href="payment.jsp?amt=4000&tut_id=<%=tutor.getTut_id()%>">BUY NOW</a>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <!-- /item -->
                                                            ...
                                                            <div class="col-md-4 text-center">
                                                                <div class="panel panel-success panel-pricing">
                                                                    ...
                                                                    <div class="panel-footer">
                                                                        <a class="btn btn-lg btn-block btn-success" href="payment.jsp?amt=5000&tut_id=<%=tutor.getTut_id()%>">BUY NOW</a>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <!-- /item -->
